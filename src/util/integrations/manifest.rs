...
use crate::util::{implants::ImplantCapability, integrations::types::UiActionDefinition};
...
impl IntegrationManifest {
    pub fn load(path: &Path) -> Result<Self> {
        let contents = fs::read_to_string(path).map_err(|err| {
            Error::new(
                err.kind(),
                format!("Unable to read integration manifest '{}': {err}", path.display()),
            )
        })?;
        serde_json::from_str(&contents).map_err(|err| {
            Error::new(
                ErrorKind::InvalidData,
                format!("Unable to parse integration manifest '{}': {err}", path.display()),
            )
        })
    }
